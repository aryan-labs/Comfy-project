...
const Product = () => {
  ...
  return (
    <div>
      ...
      <div className="p-5">
        {/* Filters Section */}
        <div className="flex flex-wrap bg-slate-100 ml-12 max-w-full sm:max-w-6xl p-5 gap-7 rounded-lg shadow-md">
          <div className="flex gap-4 flex-wrap w-full sm:w-auto">
            {/* Search Product */}
            <div className="flex flex-col w-full sm:w-auto">
              <h1 className="text-gray-600 font-semibold">Search Product</h1>
              <input
                className="border border-gray-400 rounded-lg w-full sm:w-52 p-2"
                type="text"
                name="search"
          ...
          <div className="flex gap-4 w-full sm:w-auto">
            <button
              className="h-10 w-full sm:w-52 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
              onClick={() => {}}
            >
              Search
            </button>
            <button
              className="h-10 w-full sm:w-52 bg-pink-600 text-white rounded-lg hover:bg-pink-700"
              onClick={handleReset}
            >
              Reset
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10 mt-5">
          {filteredProducts.map((product) => (
            <Link key={product.id} to={`/products/${product.id}`}>
              <div className="rounded-xl w-full h-72 border shadow-2xl flex items-center flex-col">
                <img
